#!/usr/bin/env python3
"""Run experiments with hard negative sampling for robust training."""
...
from src.aba_link_prediction.data_loaders.hard_negative_dataset import HardNegativeDataset
from src.aba_link_prediction.models import RGCN, BERTLinkPredictor, CrossEncoderBERT
...
CONFIG = {
    ...
    'hard_negative_ratio': 0.4,  # 40% hard negatives
    'structural_negative_ratio': 0.3,  # 30% structural negatives
    'random_negative_ratio': 0.3,  # 30% random negatives
    'negative_to_positive_ratio': 1.0,  # 1:1 ratio
    'similarity_threshold': 0.5,  # Threshold for hard negatives
}

def train_model_with_hard_negatives(model_type='simple_nn'):
    """Train model with hard negative sampling."""
    
    logger.info(f"Training {model_type} with hard negative sampling...")
    
    # Load datasets with hard negatives
    train_dataset = HardNegativeDataset(
        ...
        structural_negative_ratio=CONFIG['structural_negative_ratio'],
        random_negative_ratio=CONFIG['random_negative_ratio'],
        negative_to_positive_ratio=CONFIG['negative_to_positive_ratio'],
        similarity_threshold=CONFIG['similarity_threshold'],
        use_tfidf=True  # Use TF-IDF for speed
    )
    ...
    test_dataset = HardNegativeDataset(
        data_path=CONFIG['data_path'],
        mode='test',
        test_size=CONFIG['test_size'],
        val_size=CONFIG['val_size'],
        random_state=CONFIG['seed'],
        # No hard negatives for test set - keep original distribution
        hard_negative_ratio=0,
        structural_negative_ratio=0,
        random_negative_ratio=0,
        use_tfidf=True
    )
...
def visualize_results(results_dict):
    ...
    plt.tight_layout()
    plt.savefig('results/aba_link_prediction/hard_negative_results.png', dpi=100)
    logger.info("Saved visualization to results/aba_link_prediction/hard_negative_results.png")
    
    return fig

def main():
    """Run experiments with hard negative sampling."""
    ...
    # Experiment 1: Simple NN with hard negatives
    logger.info("\n" + "="*50)
    logger.info("Experiment 1: Simple NN with Hard Negatives")
    metrics, cm, report, losses, val_history = train_model_with_hard_negatives('simple_nn')
    results['SimpleNN_HardNeg'] = {
        'metrics': metrics,
        'confusion_matrix': cm,
        'report': report,
        'train_losses': losses,
    ...
    # Experiment 2: BERT with hard negatives
    logger.info("\n" + "="*50)
    logger.info("Experiment 2: BERT with Hard Negatives")
    metrics, cm, report, losses, val_history = train_model_with_hard_negatives('bert')
    results['BERT_HardNeg'] = {
        'metrics': metrics,
        'confusion_matrix': cm,
        'report': report,
        'train_losses': losses,
    ...
    # Experiment 3: Cross-Encoder BERT with hard negatives
    logger.info("\n" + "="*50)
    logger.info("Experiment 3: Cross-Encoder BERT with Hard Negatives")
    metrics, cm, report, losses, val_history = train_model_with_hard_negatives('cross_encoder')
    results['CrossEncoder_HardNeg'] = {
        'metrics': metrics,
        'confusion_matrix': cm,
        'report': report,
        'train_losses': losses,
    ...
    with open('results/aba_link_prediction/hard_negative_results.json', 'w') as f:
    ...
    # Print summary
    logger.info("\n" + "="*50)
    logger.info("HARD NEGATIVE SAMPLING EXPERIMENT SUMMARY")
    logger.info("="*50)
    ...
    logger.info("\nCurrent Results (Hard Negative Sampling):")
    for model_name, data in results.items():
        metrics = data['metrics']
    ...
    logger.info("\n✅ Hard negative sampling experiments completed successfully!")
